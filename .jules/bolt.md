...
## 2026-05-19 - Cache target indices in handleAgentEvents to prevent O(N*M) batch processing
**Learning:** When processing batches of events (like `handleAgentEvents` doing `stream_text`, `tool_call`, `tool_result`), searching the entire `messages` array for the target message (e.g. `findLastAssistantStreaming` or `findLastToolCall`) for every event in the batch results in O(N*M) time complexity. This blocks the main thread during high-frequency streaming. Furthermore, caching target indices using a single scalar variable for events that can occur multiple times per batch (like `tool_call` and `tool_result`) is insufficient, as subsequent tool calls will overwrite the index of previous ones, causing previous tool results to be dropped.
**Action:** Always extract inner loop O(N) array searches by caching target indices before the loop, updating them dynamically to achieve O(N+M) complexity. For events that can occur multiple times, always cache them using a stack or array (e.g., `pendingToolIndices.push` and `.pop`) to ensure every event is correctly mapped.
