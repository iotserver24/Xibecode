...
export default function App() {
  ...
  const handleAgentEvents = useCallback((batch: HostedAgentEvent[]) => {
    setMessages((prev) => {
      ...
      // Optimization: Cache target indices before the loop to change complexity from O(N*M) to O(N+M)
      ...
      // Reverse array so we pop the oldest pending tool first if multiple exist
      ...
      for (const event of batch) {
        ...
        switch (event.type) {
          case 'stream_text': {
            const text = d?.text ?? '';
            if (!text) break;
            if (lastAssistantIndex >= 0) updated[lastAssistantIndex] = { ...updated[lastAssistantIndex], content: updated[lastAssistantIndex].content + text };
            else {
              updated.push({ id: uid(), role: 'assistant', content: text, timestamp: event.timestamp, isStreaming: true });
              lastAssistantIndex = updated.length - 1;
            }
            break;
          }
          case 'response': {
          ...
          case 'stream_end': {
            if (lastAssistantIndex >= 0) {
              ...
              lastAssistantIndex = -1; // Explicitly invalidate
            }
            break;
          }
          case 'tool_call': {
