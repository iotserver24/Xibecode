...
export default function App() {
  ...
  const handleAgentEvents = useCallback((batch: HostedAgentEvent[]) => {
    setMessages((prev) => {
      ...
      // ⚡ Bolt: Optimize handleAgentEvents complexity from O(N*M) to O(N+M)
      // Cache target indices before the loop and update dynamically
      let lastAssistantIndex = -1;
      const pendingToolIndices: number[] = [];

      for (let i = 0; i < updated.length; i++) {
        const msg = updated[i];
        if (msg.role === 'assistant' && msg.isStreaming) lastAssistantIndex = i;
        else if (msg.role === 'tool' && msg.toolName && !msg.toolOutput) pendingToolIndices.push(i);
      }
        ...
        switch (event.type) {
          ...
          case 'stream_end': {
            if (lastAssistantIndex >= 0) {
              ...
              lastAssistantIndex = -1; // Reset to prevent regressions
            }
            break;
          }
          case 'tool_call': {
