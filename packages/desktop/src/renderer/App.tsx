...
export default function App() {
  ...
  const handleAgentEvents = useCallback((batch: HostedAgentEvent[]) => {
    setMessages((prev) => {
      ...
      // ⚡ Bolt: Cache indices to achieve O(N+M) complexity instead of O(N*M) for batch processing
      let lastAssistantIndex = -1;
      const pendingToolIndices: number[] = [];
      for (let i = 0; i < updated.length; i++) {
        if (updated[i].role === 'assistant' && updated[i].isStreaming) lastAssistantIndex = i;
        ...
        switch (event.type) {
          ...
          case 'stream_end': {
            if (lastAssistantIndex >= 0) {
              ...
              lastAssistantIndex = -1; // Reset index to prevent appending to a finished message
            }
            break;
          }
          case 'tool_call': {
          ...
          case 'tool_result': {
            const i = pendingToolIndices.pop(); // Resolve last pending tool
            if (i !== undefined && i >= 0) {
          ...
          case 'complete': {
            if (lastAssistantIndex >= 0) {
              updated[lastAssistantIndex] = { ...updated[lastAssistantIndex], isStreaming: false };
              lastAssistantIndex = -1; // Reset index
            }
            setIsRunning(false);
            break;
          }
