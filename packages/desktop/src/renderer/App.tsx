...
export default function App() {
  ...
  const handleAgentEvents = useCallback((batch: HostedAgentEvent[]) => {
    setMessages((prev) => {
      ...
      // ⚡ Bolt: Cache target indices to prevent O(N*M) time complexity from inner array searches
        ...
        switch (event.type) {
          case 'stream_text': {
            ...
            if (lastAssistantStreamingIdx >= 0) {
              updated[lastAssistantStreamingIdx] = { ...updated[lastAssistantStreamingIdx], content: updated[lastAssistantStreamingIdx].content + text };
            } else {
