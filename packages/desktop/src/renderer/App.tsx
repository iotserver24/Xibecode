...
export default function App() {
  ...
  const handleAgentEvents = useCallback((batch: HostedAgentEvent[]) => {
    setMessages((prev) => {
      ...
      // Cache targets dynamically to prevent O(N*M) lookups inside the batch
      let activeStreamingIdx = -1;
      const pendingToolIndices: number[] = [];

      for (let i = 0; i < updated.length; i++) {
        if (updated[i].role === 'assistant' && updated[i].isStreaming) {
          activeStreamingIdx = i;
      ...
      for (const event of batch) {
        ...
        switch (event.type) {
          case 'stream_text': {
            const text = d?.text ?? '';
            if (!text) break;
            if (activeStreamingIdx >= 0) {
              updated[activeStreamingIdx] = { ...updated[activeStreamingIdx], content: updated[activeStreamingIdx].content + text };
            } else {
              activeStreamingIdx = updated.length;
          ...
          case 'stream_end': {
            if (activeStreamingIdx >= 0) {
              updated[activeStreamingIdx] = { ...updated[activeStreamingIdx], isStreaming: false };
              activeStreamingIdx = -1;
            }
            break;
          }
          case 'tool_call': {
