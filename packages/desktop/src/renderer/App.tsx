...
export default function App() {
  ...
  const handleAgentEvents = useCallback((batch: HostedAgentEvent[]) => {
    setMessages((prev) => {
      ...
      // Cache target indices before the loop to achieve O(N+M) instead of O(N*M)
      ...
      // Use a stack for multiple pending tool calls
      ...
      for (const event of batch) {
        ...
        switch (event.type) {
          ...
          case 'tool_result': {
            const i = pendingToolIndices.shift(); // FIFO for tool results
            if (i !== undefined && i >= 0) {
