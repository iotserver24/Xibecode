...
export default function App() {
  ...
  const handleAgentEvents = useCallback((batch: HostedAgentEvent[]) => {
    setMessages((prev) => {
      ...
      // ⚡ Bolt: Cache target indices before the batch loop to prevent O(N*M) scanning
      // Pre-scan to find the active assistant streaming index and all pending tool calls
      let activeAssistantIdx = -1;
      const pendingToolIndices: number[] = [];

      for (let i = updated.length - 1; i >= 0; i--) {
        if (activeAssistantIdx === -1 && updated[i].role === 'assistant' && updated[i].isStreaming) {
          activeAssistantIdx = i;
        }
        // Collect all unfulfilled tool calls (pushing to front means last tool call is at end of array)
        ...
        switch (event.type) {
          case 'stream_text': {
            ...
            if (activeAssistantIdx >= 0) {
              updated[activeAssistantIdx] = { ...updated[activeAssistantIdx], content: updated[activeAssistantIdx].content + text };
            } else {
              activeAssistantIdx = updated.length;
          ...
          case 'stream_end': {
            if (activeAssistantIdx >= 0) {
              updated[activeAssistantIdx] = { ...updated[activeAssistantIdx], isStreaming: false };
          ...
          case 'tool_result': {
            // Pop the most recent pending tool call (LIFO)
            const i = pendingToolIndices.length > 0 ? pendingToolIndices.pop()! : -1;
            if (i >= 0) {
