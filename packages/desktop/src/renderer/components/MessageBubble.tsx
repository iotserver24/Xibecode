...
const MessageBubble = memo(function MessageBubble({ role, content, isStreaming }: Props) {
  ...
  if (isUser) {
    return (
      <div className="flex flex-col animate-fade-in w-full group">
        <div className="w-full text-[15px] leading-relaxed text-xibe-text whitespace-pre-wrap">
          <div className="font-semibold mb-1 text-xibe-text-dim">U</div>
          {content}
        </div>
      </div>
    );
  ...
  return (
    <div className="animate-fade-in flex flex-col w-full group">
      <div className="font-semibold mb-1 text-xibe-brand-blue">X</div><div className="prose prose-invert max-w-none text-[15px] leading-relaxed text-xibe-text
