...
const MessageBubble = memo(function MessageBubble({ role, content, isStreaming }: Props) {
  ...
  return (
    <div className="animate-fade-in flex w-full group py-4 gap-4">
      <div className="h-6 w-6 rounded flex items-center justify-center bg-xibe-surface-raised text-xs font-semibold text-xibe-text-secondary border border-xibe-border-subtle shrink-0 mt-0.5">
        {avatar}
      </div>
      ...
      <div className="flex flex-col flex-1 min-w-0">
        <div className="prose prose-invert max-w-none text-[15px] leading-relaxed text-xibe-text
          ...
          <ReactMarkdown
            ...
            components={{
              pre: ({ children }) => (
                <pre className="overflow-x-auto rounded bg-transparent p-4 text-[13px] font-mono leading-relaxed my-4">{children}</pre>
              ),
              code: ({ className, children }) => {
                const isInline = !className;
                return isInline ? (
                  <code className="rounded bg-transparent px-1.5 py-0.5 text-[13px] font-mono text-xibe-text-secondary">{children}</code>
                ) : (
