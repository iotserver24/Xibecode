  ...
  if (isUser) {
    return (
      <div className="flex justify-end animate-fade-in w-full group">
        <div className="max-w-[90%] sm:max-w-[80%] border-l-2 border-xibe-border-subtle pl-4 py-2 bg-transparent text-[15px] leading-relaxed text-xibe-text-secondary whitespace-pre-wrap">
          {content}
        </div>
      </div>
    );
    ...
    <div className="animate-fade-in flex flex-col w-full group">
      <div className="prose prose-invert max-w-none text-[15px] leading-relaxed text-xibe-text
        ...
        <ReactMarkdown
          ...
          components={{
            pre: ({ children }) => (
              <pre className="overflow-x-auto rounded-sm bg-xibe-surface p-4 text-[13px] font-mono leading-relaxed my-4">{children}</pre>
            ),
            code: ({ className, children }) => {
              const isInline = !className;
              return isInline ? (
                <code className="rounded-sm bg-xibe-surface px-1.5 py-0.5 text-[13px] font-mono text-xibe-text-secondary">{children}</code>
              ) : (
