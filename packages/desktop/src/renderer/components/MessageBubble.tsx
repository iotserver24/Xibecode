...
const MessageBubble = memo(function MessageBubble({ role, content, isStreaming }: Props) {
  ...
  return (
    <div className="animate-fade-in flex w-full group py-4">
      <div className="flex-shrink-0 mr-4 mt-0.5">
        <div className={`h-6 w-6 rounded-md flex items-center justify-center text-[11px] font-semibold ${isUser ? 'bg-xibe-text text-xibe-bg' : 'bg-xibe-brand-blue/20 text-xibe-brand-blue border border-xibe-brand-blue/30'}`}>
          {isUser ? 'U' : 'X'}
        </div>
      </div>
        ...
        {isUser ? (
          ...
          <div className="prose prose-invert max-w-none text-[15px] leading-relaxed text-xibe-text
            ...
            <ReactMarkdown
              ...
              components={{
                pre: ({ children }) => (
                  <pre className="overflow-x-auto rounded-none border-l-2 border-xibe-border-subtle pl-4 py-2 text-[13px] font-mono leading-relaxed my-4 bg-transparent">{children}</pre>
                ),
