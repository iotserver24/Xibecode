  ...
  return messages.map((msg) =>
   msg.role === 'tool' && msg.toolName ? (
    ...
    <div key={msg.id} className="flex justify-center py-2">
     <span className="text-[11px] font-medium text-xibe-text-dim bg-xibe-surface px-3 py-1 rounded-full ">{msg.content}</span>
    </div>
   ) : msg.role === 'error' ? (
    <div key={msg.id} className="text-sm text-xibe-error bg-xibe-error/10 rounded-xl px-4 py-3 ">{msg.content}</div>
   ) : (
   ...
   <div className="shrink-0 px-4 pb-6 pt-2">
    <div className="mx-auto max-w-3xl relative">
     ...
     {isSlashMode && filteredCmds.length > 0 && (
      <div className="absolute bottom-full mb-2 w-full rounded-xl bg-xibe-surface/95 backdrop-blur-md shadow-lg overflow-hidden animate-slide-up z-20">
     ...
     <div className="mt-2 text-center">
      <p className="text-[11px] font-medium text-xibe-text-dim/40">
       <span className="hidden sm:inline">Use <kbd className="font-sans px-1 rounded bg-xibe-surface/50">Enter</kbd> to send, <kbd className="font-sans px-1 rounded bg-xibe-surface/50">Shift + Enter</kbd> for new line</span>
      </p>
     </div>
    </div>
   </div>
