...
async function runCloudChatResumeBySandboxId(
  sandboxId: string,
  ...
  if (!remote) {
    throw new Error(
      "Cloud runtime is not configured. Set sandbox gateway and token first.",
    );
  }
...
program
  ...
  .option(
    "--cost-mode <mode>",
    "Cost mode: normal or economy (cheaper model, lower iteration/token caps)",
    "normal",
  )
  .option(
    "--plan-first",
  ...
  .option(
    "--non-interactive",
    "Run in non-interactive mode (for background tasks)",
    false,
  )
  .action(runCommand);
...
program
  .command("run-pr")
  .description(
    "Run an autonomous coding task, then create a branch and open a GitHub PR automatically",
  )
  .argument("[prompt]", "Task for the AI to accomplish")
  .option("-f, --file <path>", "Read prompt from file")
  .option("-m, --model <model>", "AI model to use")
  ...
  .option(
    "--cost-mode <mode>",
    "Cost mode: normal or economy (cheaper model, lower caps)",
    "normal",
  )
  .option(
    "--plan-first",
  ...
  .option(
    "--branch <name>",
    "Override branch name (default: auto-generated xibecode/<slug>-<timestamp>)",
  )
...
program
  ...
  .option(
    "--plain",
    "Disable Ink UI; print line-by-line output (best for copying)",
    false,
  )
...
// Models command - list available models from API
...
cloudCmd
  ...
  .option(
    "--output <path>",
    "Destination directory (default: .xibecode/sandbox-pull-<timestamp>)",
  )
  .option(
    "--apply",
    "Merge sandbox files into current working directory",
    false,
  )
  .option(
    "--full",
...
resumeCmd
  ...
  .argument(
    "[session-id]",
    "Session ID to resume (optional - shows picker if not provided)",
  )
  .option(
    "--profile <name>",
    "Config profile to use (default: configured default profile)",
  ...
  .action(
    (
      ...
      options: { profile?: string; all?: boolean },
    ) => {
      process.env.XIBECODE_SANDBOX_MODE = "local";
      resumeCommand({
        session: sessionId,
...
program
  ...
  .option(
    ...
    "Set default provider/API format (anthropic, openai, deepseek, zai, kimi, grok, openrouter, google, auto)",
  )
  .option(
    "--set-cost-mode <mode>",
    "Set cost mode: normal or economy (use cheaper model and lower caps)",
  )
  .option(
    "--set-economy-model <model>",
    "Set model to use when cost mode is economy",
  )
  .option(
    "--set-sandbox-mode <mode>",
    "Set command execution mode: local or e2b",
  )
  .option(
    "--set-sandbox-gateway-url <url>",
    "Set team gateway URL used for E2B-backed command execution",
  )
  .option(
    "--set-sandbox-auth-token <token>",
    "Set optional team gateway bearer token (not E2B API key)",
  )
  .option(
    "--set-sandbox-session-strategy <strategy>",
    "Set sandbox session strategy: host_only or sandbox_full",
  )
  .option(
    "--set-sandbox-sync-max-mb <number>",
    "Set max compressed workspace size for sandbox_full sync (MB)",
  )
  .option(
    "--set-sandbox-sync-exclude <csv>",
    "Set comma-separated exclude globs for sandbox_full sync tarball",
  )
  .option(
...
program
  .command("diagnostics")
  .description(
    "Generate a diagnostics bundle for bug reports (secrets are redacted)",
  )
  .option(
    "-o, --output <path>",
    "Output file path (default: tmp/xibecode-diagnostics-<timestamp>.md)",
  )
  .option(
    "--profile <name>",
    "Config profile to use (default: configured default profile)",
  ...
  .option(
    "--include-diff",
    "Include full unified git diff (redacted, may be truncated)",
    false,
  )
...
program
  ...
  .action((action: string | undefined, args: string[], options: any) =>
    skillsCommand(action, args, options),
  );
...
program
  ...
  .action((action: string | undefined, args: string[], options: any) =>
    settingsCommand(action, args, options),
  );
...
program
  ...
  .option("--profile <name>", "Config profile to use")
  .action((action: string | undefined, args: string[], options: any) =>
    hooksCommand(action, args, options),
  );
...
if (acpArgIdx >= 2) {
  ...
  const profile =
    profileIdx >= 0 && profileIdx + 1 < process.argv.length
