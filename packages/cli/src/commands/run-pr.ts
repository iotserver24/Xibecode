...
import { NeuralMemory } from 'xibecode-core';
import { SessionMemory } from 'xibecode-core';
import { pruneContext } from 'xibecode-core';
...
export async function runPrCommand(prompt: string | undefined, options: RunPrOptions) {
  ...
  const skillManager = new SkillManager(cwd, apiKey, baseUrl, model, provider, builtInSkillsDir);
  ...
  if (autoSkillsShEnabled) {
    const auto = await skillManager.autoInstallFromSkillsShForTask(finalPrompt, {
      enabled: true,
      maxInstalls: 1,
  ...
  let defaultSkillsPrompt = await skillManager.buildDefaultSkillsPromptForTask(finalPrompt, cwd);
