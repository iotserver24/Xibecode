...
export class PlanMode {
  ...
  async buildPlan(description: string): Promise<PlanResult> {
    ...
    architect.on('event', (event: AgentEvent) => {
      if (event.type === 'thinking') console.log(`  ${event.data.message}`);
    });
...
Perform these steps now.
    ...
    try {
      ...
      console.error('Architect failed to generate plan:', error);
    }
    ...
    if (doc.pending.length === 0 && doc.inProgress.length === 0 && doc.done.length === 0) {
      console.log('  (Architect did not create tasks, falling back to heuristic)');
